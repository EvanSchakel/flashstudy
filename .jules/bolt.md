...
## 2026-05-29 - Unnecessary Disk I/O on Unmodified State
**Learning:** For interactive CLI operations (like study sessions), blindly saving state at the end of the operation causes unnecessary disk writes if the user exits early without actually modifying any state (e.g., zero cards attempted).
**Action:** Always track if an entity's state was actually modified during an interactive session and use early returns or conditional checks to skip the save operation if the state is unmodified.
