...
## 2026-07-02 - Avoid Unnecessary Disk I/O on Unmodified State
**Learning:** Saving data to disk during an interactive session (e.g., studying flashcards) after every session unconditionally results in unnecessary disk I/O if the user aborted the session before making any changes.
**Action:** Track whether an entity's state was actually modified during an interactive session and use early returns to skip save operations if the state is unmodified.
