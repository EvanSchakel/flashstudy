...
## 2026-06-20 - Skip Unnecessary Saves During Interactive Sessions
**Learning:** Automatically saving state after an interactive session (e.g., studying a deck) causes unnecessary disk I/O if the user aborted immediately or completed zero actions. By tracking if state was actually modified, we can avoid this overhead.
**Action:** Always track whether an entity's state was actually modified during an interactive flow and use early returns to conditionally skip expensive save/persistence operations.
