...
## 2026-06-11 - Unnecessary Disk I/O on Aborted Sessions
**Learning:** Unconditionally saving state (e.g., a deck) after an interactive session, even when the user exits early without making any modifications (0 cards attempted), causes redundant disk I/O and serialization overhead.
**Action:** Track whether an entity's state was actually modified during an interactive session and use early returns to skip expensive save operations if the state is unmodified.
