...
## 2026-06-04 - Unmodified State Persistence Skipping
**Learning:** In interactive CLI applications, automatically persisting state (e.g., saving a deck to disk) after a session completes creates unnecessary disk I/O if the user aborted immediately or made no actual changes (like skipping all flashcards).
**Action:** Always track whether the entity's state was actually modified during the session (e.g., tracking `attempted > 0`), and use early returns to conditionally skip save operations if the state is unmodified.
