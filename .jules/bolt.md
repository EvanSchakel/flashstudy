...
## 2026-05-28 - Skip Redundant Disk I/O on Unmodified Entities
**Learning:** Interactive sessions (like studying flashcards) may conclude without the user actually modifying any underlying state (e.g., exiting before answering any cards). Saving unconditionally in these cases causes unnecessary disk I/O overhead.
**Action:** Always track whether an entity's state was actually modified during an interactive session (using boolean returns or dirty flags) and use early returns to skip redundant save operations to avoid unnecessary disk I/O.
