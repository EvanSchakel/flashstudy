...
## 2026-06-30 - Skip unneeded disk I/O on aborted sessions
**Learning:** Interactive sessions (like study flows) often end prematurely, causing the default `save()` at the end of the flow to perform redundant disk I/O for unmodified entities.
**Action:** Track session modifications (e.g., if any cards were attempted) and skip the final save operation if the state remains unmodified.
