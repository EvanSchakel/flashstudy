...
## 2026-07-04 - Early Return on Unmodified State
**Learning:** During interactive sessions (like studying a deck), an entity's state may not actually change if the user aborts early (e.g., studying 0 cards). Blindly saving the entity after every session incurs unnecessary disk write overhead and cache invalidation/update cycles.
**Action:** Always track whether the entity was actually modified during the interaction and use early returns to skip expensive save operations if the state is unmodified.
