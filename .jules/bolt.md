...
## 2024-05-24 - Skip saving unmodified interactive state
**Learning:** The CLI was eagerly saving deck state after every study session, even when the user aborted before attempting any cards. This caused unnecessary I/O overhead.
**Action:** When tracking interactive modifications, return a boolean from the session runner to indicate if the entity state actually changed, and use an early return/conditional to skip the expensive save operation if it was unmodified.
