...
## 2026-06-16 - Prevent Unnecessary Disk Writes on Unmodified State
**Learning:** Found that CLI study sessions trigger a full JSON rewrite of the deck on disk every single time the study option is selected, even if the user immediately cancels the session without attempting any cards. This occurs because the `studyDeck` method blindly saves the deck after `StudySession.run()` finishes. In interactive environments with persisted cache loops, unconditional disk writes can cause significant unnecessary I/O overhead.
**Action:** When tracking updates for interactive sessions that may end up not modifying state, always use a return value or flag to early-exit the disk save operation if the state is unmodified.
