...
## 2026-06-06 - Skip Unnecessary Disk I/O
**Learning:** Always saving entities to disk after an interactive session, regardless of whether the user actually performed an action that modified state (e.g., studying zero cards), results in unnecessary disk I/O and file writes.
**Action:** Track whether an entity's state was actually modified during a session (e.g., tracking attempted items) and use early returns to skip `save` operations if the state is unmodified.
