...
## 2026-06-27 - Fix TOCTOU Vulnerability in File Creation
**Vulnerability:** File.createNewFile() followed by File.setReadable() creates a Time-of-Check to Time-of-Use (TOCTOU) race condition and Symlink Race risk.
**Learning:** Using java.io.File for secure file creation is inherently vulnerable. Permissions must be applied atomically at creation time.
**Prevention:** Use Files.createFile() with PosixFilePermissions for atomic file creation. Always handle UnsupportedOperationException for non-POSIX (Windows) environments as a fallback. When writing to these files, use Files.newOutputStream with LinkOption.NOFOLLOW_LINKS to prevent data corruption.
