...
## 2026-06-17 - Fix TOCTOU vulnerability in file creation
**Vulnerability:** The `saveDeck` method in `DeckStore` used `File.createNewFile()` followed by `File.setReadable()`, which is vulnerable to Time-of-Check to Time-of-Use (TOCTOU) / Symlink Race (CWE-367) attacks. An attacker could replace the file with a symlink between creation and permission setting, potentially altering the permissions of a different file or reading data they shouldn't.
**Learning:** `java.io.File` methods are generally not atomic and safe for security-sensitive file operations.
**Prevention:** Instead, create files atomically using `Files.createFile(path, PosixFilePermissions.asFileAttribute(perms))` with strict permissions `rw-------`. Always use `StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.WRITE, LinkOption.NOFOLLOW_LINKS` with `Files.newOutputStream` to avoid symlink traversal and properly overwrite existing content. Add a fallback for non-POSIX systems by catching `UnsupportedOperationException` and using the less secure `File` methods as a last resort. Keep in mind `PosixFileAttributeView.class` must be requested with `LinkOption.NOFOLLOW_LINKS` when updating existing file permissions.
