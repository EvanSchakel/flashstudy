...
## 2026-06-14 - Fix TOCTOU (Time-of-Check to Time-of-Use) Vulnerabilities in File Creation
**Vulnerability:** The application was using `File.createNewFile()` followed by `File.setReadable()` to enforce file permissions. This created a race condition where an attacker could replace the newly created file with a symbolic link before permissions were applied, potentially leading to unauthorized access, file corruption, or privilege escalation (Symlink Race, CWE-367).
**Learning:** `File.createNewFile()` and `File.setReadable()` are not atomic operations. In Unix-like environments, `Files.createFile()` should be used with `PosixFilePermissions` to create files atomically with the correct permissions from the start.
**Prevention:** Always use atomic operations for file creation with permissions. Avoid `java.io.File` and use `java.nio.file.Files.createFile(path, PosixFilePermissions.asFileAttribute(perms))` with fallback logic for non-POSIX systems to prevent TOCTOU vulnerabilities.
