...
## 2024-05-15 - TOCTOU Symlink Race Conditions in File Creation
**Vulnerability:** Found Time-of-Check to Time-of-Use (TOCTOU) vulnerabilities in `DeckStore.java` and `Main.java` where `File.createNewFile()` was followed by `File.setReadable()`. An attacker could exploit this by creating a symlink in the time gap, causing permissions to be applied to a sensitive target file.
**Learning:** Atomic file creation with correct initial permissions is critical on POSIX systems to prevent symlink attacks. Additionally, when bypassing symlinks via `Files.newOutputStream(..., LinkOption.NOFOLLOW_LINKS)`, explicit open options (`CREATE`, `TRUNCATE_EXISTING`, `WRITE`) must be provided because the default options are omitted.
**Prevention:** Always use atomic `Files.createFile` with `PosixFilePermissions`. Wrap it in a try-catch for `UnsupportedOperationException` for Windows fallback, where permissions should be strictly set inside the boolean check of `createNewFile()`.
