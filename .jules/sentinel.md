...
## 2026-06-18 - Fix TOCTOU vulnerability in file creations

**Vulnerability:** `File.createNewFile()` followed by `File.setReadable()` is a Time-of-Check to Time-of-Use (TOCTOU) vulnerability that can lead to Symlink Race attacks. The same applied to directory creation via `Files.createDirectories` where permissions were set after creation.
**Learning:** `java.io.File` doesn't provide atomic file creation with specific permissions. While `Files.createFile` with `PosixFilePermissions` provides atomic secure file creation, it must be combined with a fallback for non-POSIX systems (like Windows). Also, replacing `FileWriter` with `Files.newOutputStream(..., LinkOption.NOFOLLOW_LINKS)` requires explicitly setting `StandardOpenOption.CREATE`, `StandardOpenOption.WRITE`, and either `StandardOpenOption.APPEND` or `StandardOpenOption.TRUNCATE_EXISTING` when providing any specific open option.
**Prevention:** Use `Files.createFile` with `PosixFilePermissions` inside a try-catch for `UnsupportedOperationException` for secure atomic creation. Use `Files.newOutputStream` with explicit `StandardOpenOption`s and `LinkOption.NOFOLLOW_LINKS` to safely open files.
