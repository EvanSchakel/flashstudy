...
## 2026-05-29 - Prevent TOCTOU Race Condition on File Creation
**Vulnerability:** The application used `!logFile.exists()` to check if a log file exists before trying to create it via `createNewFile()`, and then applied restrictive permissions unconditionally. This introduces a Time-of-Check to Time-of-Use (TOCTOU) race condition where a malicious actor could create a symlink or an actual file in that brief window, leading the application to apply strict permissions to an arbitrary file or a file they control.
**Learning:** File system checks before creation are not atomic and lead to race conditions.
**Prevention:** Avoid `!exists()` checks before file creation. Instead, leverage the atomic boolean return value of `File.createNewFile()`, which returns `true` only if the file did not exist and was successfully, atomically created by this thread. Apply restrictive file permissions only if `createNewFile()` returns `true`.
