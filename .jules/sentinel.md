...
## 2026-06-08 - Use java.nio.file.Files for Secure File Creation
**Vulnerability:** File creation used `File.createNewFile()` followed by `setReadable(false, false)`, leaving a window where the file existed with default (potentially overly permissive) OS permissions before they were restricted, which is a TOCTOU race condition and vulnerable to Symlink Race (CWE-367) attacks.
**Learning:** `java.io.File` methods are inadequate for atomic, secure file creation. `createNewFile()` cannot set initial permissions, and modifying them later leaves a vulnerable time window. Additionally, `FileWriter` blindly follows symlinks, compounding the risk.
**Prevention:** To prevent Symlink Race (CWE-367) attacks and TOCTOU vulnerabilities when creating sensitive files (like error logs), use `java.nio.file.Files.createFile(path, PosixFilePermissions.asFileAttribute(perms))` for atomic creation with strict permissions, and write using `Files.newOutputStream` with `LinkOption.NOFOLLOW_LINKS` to ensure safe data appending.
