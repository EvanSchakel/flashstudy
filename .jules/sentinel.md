...
## 2026-06-01 - Prevent Symlink Race (TOCTOU) during Log Creation
**Vulnerability:** The application used `if (!logFile.exists()) { logFile.createNewFile(); }` followed by unconditional file permission assignments (`setReadable(false, false)`, etc.). This introduces a Time-of-Check to Time-of-Use (TOCTOU) / Symlink Race vulnerability, where an attacker could create a symlink at the target path after the existence check, causing the restrictive permissions to be applied to the symlink target instead.
**Learning:** Unconditionally applying permissions to a file after creation without verifying atomic creation can lead to race conditions where permissions are applied to unintended files (like attacker-controlled symlinks).
**Prevention:** Always wrap permission-setting methods (e.g., `setReadable(false, false)`) strictly within the conditional block where `createNewFile()` returns `true` (e.g., `if (logFile.createNewFile()) { logFile.setReadable(...) }`) to prevent Symlink Race (CWE-367) attacks and ensure atomic operations.
