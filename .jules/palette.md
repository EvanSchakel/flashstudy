...
## 2024-05-21 - Add explicit escape hatch to long-running CLI prompts
**Learning:** Users can feel trapped in long-running CLI interactive loops (like studying a large deck) if there isn't a clearly documented way to exit early. This forces unnatural exits (like Ctrl+C).
**Action:** Always provide and clearly document an explicit escape hatch (e.g., pressing 'q' to quit) in long-running CLI interactive prompts, and gracefully handle early termination (e.g., calculating stats based on attempted items instead of total items).
