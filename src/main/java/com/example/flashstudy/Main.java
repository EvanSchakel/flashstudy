...
public class Main {
    ...
    private static void logError(Throwable t) {
        try {
            ...
            // 🛡️ Sentinel: Prevent TOCTOU symlink race by creating error log atomically
            try {
                java.util.Set<java.nio.file.attribute.PosixFilePermission> p = java.nio.file.attribute.PosixFilePermissions.fromString("rw-------");
                Files.createFile(errorLogPath, java.nio.file.attribute.PosixFilePermissions.asFileAttribute(p));
            } catch (java.nio.file.FileAlreadyExistsException e) {
                java.nio.file.attribute.PosixFileAttributeView v = Files.getFileAttributeView(errorLogPath, java.nio.file.attribute.PosixFileAttributeView.class, java.nio.file.LinkOption.NOFOLLOW_LINKS);
                if (v != null) v.setPermissions(java.nio.file.attribute.PosixFilePermissions.fromString("rw-------"));
            } catch (UnsupportedOperationException e) {
                java.io.File logFile = errorLogPath.toFile();
                if (logFile.createNewFile()) {
                    logFile.setReadable(false, false); logFile.setWritable(false, false); logFile.setExecutable(false, false);
                    logFile.setReadable(true, true); logFile.setWritable(true, true);
                }
            }
            ...
            try (java.io.OutputStream os = Files.newOutputStream(errorLogPath, java.nio.file.StandardOpenOption.CREATE, java.nio.file.StandardOpenOption.APPEND, java.nio.file.StandardOpenOption.WRITE, java.nio.file.LinkOption.NOFOLLOW_LINKS);
                 PrintWriter pw = new PrintWriter(new java.io.OutputStreamWriter(os, java.nio.charset.StandardCharsets.UTF_8), true)) {
